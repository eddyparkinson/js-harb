...
var csv_to_aoa = function (str, opts) {
  var keys = Object.keys(bpopts);
  var o = {};
  ...
  var data = babyParse.parse(str, o).data;
...
var csv_to_sheet = function (str, opts) { return aoa_to_sheet(csv_to_aoa(str, opts)); };

var csv_to_workbook = function (str, opts) { return sheet_to_workbook(csv_to_sheet(str, opts)); };
...
var read = function (f, opts, hint) {
  if(hint === 0xFFFE) return csv_to_workbook(f, opts);
  ...
  else if(f.indexOf(',') != -1 || f.indexOf('\t') != -1) return csv_to_workbook(f, opts);
